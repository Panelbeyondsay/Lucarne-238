...
fn codex_agent_session_benchmarks(c: &mut Criterion) {
    ...
    for sample in &samples {
        ...
        assert!(
            full.events.len() >= messages.events.len(),
            "message selection produced more events than full selection for {}",
            sample.path.display()
        );
    }

    let mut group = c.benchmark_group("codex_agent_session");
    ...
    for sample in &samples {
        ...
        for (name, selection) in [
            ("parse_full_agent_session", ParseSelection::full()),
            (
                "parse_message_agent_session",
                ParseSelection::empty().with_messages(),
            ),
        ] {
...
criterion_group!(benches, codex_agent_session_benchmarks);
