...
fn claude_benchmarks(c: &mut Criterion) {
    let provider = agent_sessions::agent_provider("claude").expect("claude provider");
    let samples = load_samples();
    assert!(
        !samples.is_empty(),
        "no Claude session samples found; set AGENT_SESSIONS_BENCH_CLAUDE_SAMPLES or populate ~/.claude/projects"
    );

    let mut group = c.benchmark_group("claude_parse");
...
fn explicit_sample_paths() -> Option<Vec<PathBuf>> {
    let raw = env::var_os("AGENT_SESSIONS_BENCH_CLAUDE_SAMPLES")?;
...
criterion_group!(benches, claude_benchmarks);
